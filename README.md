![Header](Assets/images/githubBanner6.png)
<p align="center">
  🔍 <strong>Focus:</strong> Software Quality Assurance • Defect Lifecycle Management • Accessibility (WCAG) Auditing • Business Logic Validation<br>
  🛠️ <strong>Tools & Technologies:</strong> Chrome DevTools • Postman (API) • SQL • Java • JUnit • Jira<br>
  🔧 <strong>Practicing:</strong> Exploratory Testing • Boundary Value Analysis • Bug Reporting • UI/UX Integrity
</p>

---

<h3 align="center">Core Tech Stack</h3>
<p align="center">
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=java&logoColor=white" alt="Java" height="28"/> 
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python" height="28"/> 
  <img src="https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white" alt="Postman" height="28"/> 
  <img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white" alt="SQL" height="28"/> 
  <img src="https://img.shields.io/badge/Jira-0052CC?style=for-the-badge&logo=Jira&logoColor=white" alt="Jira" height="28"/> 
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" alt="HTML5" height="28"/> 
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" alt="CSS3" height="28"/> 
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" alt="Linux" height="28"/> 
</p>

--- 

<h3 align="center">Achievements & Certifications</h3>

<p align="center">
<img src="Assets/images/mtaLogo.png" alt="MTA 98-361 Cert" width="120"/> 
  <img src="Assets/images/pcapLogo.png" alt="PCAP Cert" width="120"/>
  <img src="Assets/images/sc900.png" alt="SC900 Cert" width="120"/>
</p>

<br /> 

<div align="center">
  <a href="https://tryhackme.com/p/katyfa"> 
    <img src="https://tryhackme-badges.s3.amazonaws.com/katyfa.png" alt="TryHackMe Badge"/>
  </a>
</div>

---

<details>
  <summary><strong>📃 CV & Experience</strong></summary>

  <h2>Education & Projects</h2>

  * **Level 4 Software Development and Testing** - _Distinction_ <br>
      📆 Jan - Mar 2025 | 📍 Gateshead College <br>
      *Project Focus:* Executed structured manual testing against business requirements. Authored comprehensive defect reports and successfully re-architected a banking application to eradicate floating-point calculation errors using BigDecimal.

  * **Software Development Fundamentals Bootcamp**<br>
      📆 Oct - Feb 2022 | 📍 Firebrand
      
 <h2>Certifications</h2>
   
  * **Microsoft Security, Compliance and Identity Fundamentals (SC-900)** | 📆 Apr 2025  
  * **Microsoft Technology Associate: Software Development Fundamentals (MTA 98-361)** | 📆 Feb 2022  
  * **PCAP – Certified Associate in Python Programming** | 📆 Feb 2022  
      
  <h2>💼 Work Experience</h2>
  
  * **Independent Application QA & E-Commerce Management**<br>
      📆 Jan 2022 – Present<br>
      *Key Responsibilities:* Acted as lead QA for an independent AI-assisted application build, executing manual exploratory testing across multiple breakpoints using DevTools. Managed e-commerce operations, strictly enforcing inventory data accuracy and optimizing UI/UX purchasing flows.

  * **Customer Service Advisor (Government Contracts)**<br>
      📆 Sep 2022 – May 2025 | 📍 G4S  <br>
      *Key Responsibilities:* Navigated complex internal software systems to manage a high volume of requests, identifying edge cases and user-flow friction points. Enforced strict data integrity and regulatory compliance across government accounts.

  * **Customer Advisor**<br>
      📆 Feb 2020 – Jan 2022 | 📍 Student Loans Company  <br>
      *Key Responsibilities:* Utilized diagnostic questioning to resolve complex inquiries. Maintained strict adherence to GDPR and data confidentiality protocols while handling sensitive financial data.
      
</details>

---

<h4 align="center">Reach Me</h4>  
<p align="center">
  <a href="mailto:katy_a_cv.heroics112@slmail.me"><img src="https://img.shields.io/badge/Tutanota-840010?style=for-the-badge&logo=Tutanota&logoColor=white" alt="Tutanota"></a>
  <span>&nbsp;</span>
  <a href="https://www.linkedin.com/in//"><img src="https://img.shields.io/badge/linkedin-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"></a>
</p>

<!---
KatyFrancesA/KatyFrancesA is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
